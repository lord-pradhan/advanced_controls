...
float theta_filt_prev, alpha, beta, theta_dot_prev;

/* Controller variables */
float x_ref[4]={0.0,0.0,0.0,0.0}; 
float k_fb[4]={0.0,0.0,0.0,0.0}; 
...
long accelXCalli = 2, accelYCalli = 174, accelZCalli = (16316 - 16384); // Obtained by callibrating accelerometer values
...
void getActualIMU()
{
    ...
    meas_wx = (float) gx *  3.1415 / (180.0 * 131.0);
    meas_wy = (float) gy *  3.1415 / (180.0 * 131.0);
    meas_wz = (float) gz *  3.1415 / (180.0 * 131.0);
}
...
//  Serial.println("Printing th/rough USB Serial Port");
//  BTserial.println(/"Printing through Bluetooth");
  ...
  // estimate full state
  float theta_dot = beta*meas_wx + (1-beta)*theta_dot_prev;  
  thetaValLeft += (float) motor_left_ang_vel * (time - prev_time) / 1000.0;
  float x_meas = -wheel_rad*thetaValLeft;
  float xdot_meas = -wheel_rad*motor_left_ang_vel;

  BTserial.print(meas_wx);
  BTserial.print(",");
  BTserial.println(theta_dot);

  // calc errors
  float x_err = x_meas - x_ref[0];
  float xdot_err = xdot_meas - x_ref[1];
  float theta_err = theta_filt - x_ref[2];
  float thetadot_err = theta_dot - x_ref[3];

  // calc control effort
  float volt = -k_fb[0]*x_err - k_fb[1]*xdot_err - k_fb[2]*theta_err - k_fb[3]*thetadot_err;
  float pwm = max(min(volt / 8.0 * 255.0, 255.0), -255.0);
  SetLeftWheelSpeed(pwm);
  SetRightWheelSpeed(pwm);
  
//  BTserial.print(-k_fb[2]*theta_err);
//  BTserial.print(",");
//  BTserial.println(-k_fb[3]*thetadot_err);
////  BTserial.println(',');  
//  BTserial.println('\n');  
